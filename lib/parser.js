        ...
        return new Promise(function(resolve, reject) {
            ...
            var converter = jsonMapper.makeConverter({
                ...
                armedStatus: function(input) {
                    // Status is numeric: 0=disarmed, 1=armed, 2=partialArmed
                    var status = input.Status;
                    if (status === 0) return 'disarmed';
                    if (status === 1) return 'armed';
                    if (status === 2) return 'partialArmed';
                    return 'unknown';
                },
                annexArmedStatus: function (input) {
                    // AnnexStatus is numeric: 0=disarmed, 1=armed
                    var status = input.AnnexStatus;
                    if (status === 0) return 'disarmed';
                    if (status === 1) return 'armed';
                    return 'unknown';
                }
    ...
    transformHistoryToOutput(historyJson, top) {
        return new Promise(function(resolve, reject) {
            ...
            try {
                ...
                // Transform each event to the expected format
                var transformedEvents = events.slice(0, top).map(function(event) {
                    return {
                        time: moment(event.Time).format('YYYY-MM-DD HH:mm:ss'),
                        action: function() {
                            if (event.EventType == 'partialarmed')
                                return 'partialArmed';
                            else if (event.EventType == 'armedannex')
                                return 'armedAnnex';
                            else if (event.EventType == 'disarmedAnnex')
                                return 'disarmedAnnex';
                            else
                                return event.EventType;
                        }(),
                        user: function() {
                            if (event.User == 'Kod' || event.User == 'kode')
                                return 'Code';
                            else
                                return event.User || '';
                        }()
